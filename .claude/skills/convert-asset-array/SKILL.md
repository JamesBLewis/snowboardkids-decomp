...
3. Update `snowboardkids.yaml` to add named segments with proper end markers
...
grep "_224420:" snowboardkids.yaml

# Find all uses of an address
grep -r "0x00224420" src/

# Validate segment order
python3 -m splat split snowboardkids.yaml
...
with open('snowboardkids.yaml', 'r') as f:
    lines = f.readlines()
...
3. Check the YAML segments around those addresses:
   ```bash
   grep -n "0xADDRESS" snowboardkids.yaml
   ```
