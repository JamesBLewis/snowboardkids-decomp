...
description: Adding a new asset to snowboardkids.yaml requires following specific steps to ensure assets are in the correct order and properly declared. Use this skill when you need to add a new binary asset.
---

# Add a new asset to snowboardkids.yaml

## Overview

Binary assets (data blobs extracted from the ROM) must be defined in `snowboardkids.yaml` before they can be used in C code via the `USE_ASSET()` macro. This skill documents how to identify when an asset needs to be added and how to add it correctly.
...
Search `snowboardkids.yaml` for the asset name. Asset names follow the pattern `_<hex_address>`:

```bash
grep "_45B130:" snowboardkids.yaml
```
...
### 2. Add or convert the asset in snowboardkids.yaml
