...
- **Symbol-based lookup**: Uses `symbol_addrs.txt` to find data symbols; single-symbol diffs need `rom:` metadata or a supplied size
- **Byte-by-byte comparison**: Shows exact byte differences
- **Size detection**: Reads size from `symbol_addrs.txt` when annotated, or accepts `--size`
...
python3 tools/data-differ/data_diff.py <symbol> --size 0x10

# Show all data even if it matches
python3 tools/data-differ/data_diff.py <symbol> --show-all

# Custom ROM paths
python3 tools/data-differ/data_diff.py <symbol> \
  --target-rom snowboardkids.z64 \
  --compiled-rom build/snowboardkids.z64
```
...
python3 tools/data-differ/data_diff.py --find-first-mismatch --filter D_8008
...
Found mismatch at symbol #23: <symbol>
...
Example:
```
exampleDataSymbol = 0x8008D9C0; // rom:0x8E5C0 size:0x10
